...
function getCurrentPetalPosition(petal) {
  return {
    cy: parseFloat(petal.getAttribute('cy')),
    rx: parseFloat(petal.getAttribute('rx')),
    ry: parseFloat(petal.getAttribute('ry')),
  };
}

function updatePetalPosition(petal, state) {
  petal.setAttribute('cy', state.cy);
  petal.setAttribute('rx', state.rx);
  petal.setAttribute('ry', state.ry);
}
...
function calculatePetalPosition(startPosition, targetPosition, progress) {
  return {
    ...
    rx: startPosition.rx + (targetPosition.rx - startPosition.rx) * progress,
    ry: startPosition.ry + (targetPosition.ry - startPosition.ry) * progress,
  };
}
